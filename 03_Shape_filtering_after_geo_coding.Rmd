...
```{r}
IRS_balt <-left_join(in_balt, IRS, by = "ein") #sf version - need to make a version like this without the sf version start here avocado
...
```{r}
IRS_with_revonly <-inner_join(in_balt_IRS, revocations, by = "ein")
IRS_with_revonly  <- as_tibble(IRS_with_revonly)
...
IRS_with_revonly <- IRS_with_revonly %>% mutate(tax_period_asdate = ym(tax_period), # IRS data
                                                rev_date1_asdate = dmy(rev_date1.x), # rev data
                                                rev_date2_asdate = dmy(rev_date2.x), # rev data
...
```{r}
IRS_no_revoc<-anti_join(in_balt_IRS, ein_to_rem, by = "ein") # same dimensions none to remove
IRS <-IRS_no_revoc

write_csv(in_balt_IRS, file = "New_version_data/clean_data/balt_irs.csv")
write_csv(IRS_no_revoc, file = "New_version_data/clean_data/balt_irs_no_revoc.csv")
