...
To compare with and without outliers and with and without zeros
```{r}


# trying again without outliers for 
...
lm2 <- lm(log(F990_TOTAL_ASSETS_RECENT) ~ poly(Percent_AA, 2) , data = df_simplified)
summary(lm2) #need plus 1 for log of zero
ggplot(df_simplified, aes(poly(Percent_AA,2), log(F990_TOTAL_ASSETS_RECENT))) +
  geom_point(alpha = 0.2) +
  geom_smooth(method = "loess")
