...
outliers <- boxplot(df_simplified_no_zero$F990_TOTAL_ASSETS_RECENT, plot=FALSE)$out
df_simplified_no_zero_no_outliers <- filter(df_simplified_no_zero, !F990_TOTAL_ASSETS_RECENT %in% outliers)
...
```{r}
# Check different models for scaling percent_AA, including or not including population and removing outliers
...
# try again with outlier removal
...
AIC(lm1)
AIC(lm2) # much lower so possibly better
summary(lm1)$adj.r.squared # slightly higher so possibly better model
summary(lm2)$adj.r.squared 

#hard to conclude but can use in sensitivity analysis
...
## What % of the total baltimore population live in majority black neighborhoods


```{r}

Black_majority_neighborhoods <-neighborhood_shape%>% mutate(Percent_AA = 
...
Not_Black_majority_neighborhoods <-neighborhood_shape%>% mutate(Percent_AA = 
           round(Blk_AfAm/Population*100, digits = 2)) %>% filter(Percent_AA<50) %>% pull(Name)
...
#population living in Black majority neighborhoods
...
#population living in not living in Black majority neighborhoods
pop_not_in_black_neighborhoods

pop_in_black_neighborhoods/(sum(pop_in_black_neighborhoods+pop_not_in_black_neighborhoods))*100
