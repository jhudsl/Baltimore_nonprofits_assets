...
From Tyler:
Here’s an article describing some of the datasets from the IRS 990s.  https://nccs.urban.org/sites/default/files/2018-12/Guide%20to%20Using%20NCCS%20Data.pdf
We’re using the BMF.  There’s a section titled “Minimum Filing Threshold” that explains a data limitation (and why we’re seeing so many 0’s).  With this confirmation, we may want to return to Asset Codes, even though it’s ordinal.  Carrie, any way of maintaining scale data while managing this limitation? I can’t think of one.
Thanks,
Tyler

It seems that if there is a value less than 50,000 other than zero, it must mean that the organization decided to submit to the IRS, becuase otherwise they would be listed as a zero. It is not possible to distinguish a true zero from a zero due to not meeting the threshold of 50,000 and thus not submitting.

I think it therefore makes sense to remove zero values (which I have already done) and to report this caveat that the data is incomplete because many nonprofits that had assets less than 50,000 are not included.
...
Next we will modify the data to include a variable about the percentage of African American or Black people.
```{r}
 ...
 df_simplified<-df_simplified %>%
  ...
  # create a new variable about this in text
   mutate(Neighborhood = case_when(
    ...
    Percent_AA <  50 ~ "Majority\nNon-Black")) %>% # Tyler thinks if a neighborhood is less than 50% black that they are majority white from previously looking at the data and is checking that this is true... that there aren't some neighborhoods that might have a different composition
...
### Quantile data
To group the data by quantiles, we first remove organizations with zero assets because we don't know if zero values are real. If assets are under 50,000 I think organizations can report as zero also.
...
```{r}
#nonparmetric test
cor.test(df_simplified_no_zero$ASSET_AMT, df_simplified_no_zero$Percent_AA, method = "spearman")
