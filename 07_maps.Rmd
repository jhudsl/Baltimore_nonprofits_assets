...
Todo:
- Remove orgs in industrial neighborhoods with no population
- convert "NA" to actual `NA`.

```{r}
Nieghborhoods_no_pop <-neighborhoods %>% filter(Population == 0) %>% pull(Name)
df_simplified %>% no
df_simplified %>% filter(Neighborhood %in% Nieghborhoods_no_pop)
```
...
neighborhoods_sf <- neighborhoods_sf %>% mutate(Percent_AA
 ...
 neighborhood_dem <- ggplot() +geom_sf(data = neighborhoods_sf, aes(fill = Neighborhood_type))+
    theme_minimal() +
   labs(title = "Neighborhood Demographics")
 neighborhood_dem
...
asset_plot <- neighborhood_dem + 
    geom_sf(data = multi_polygon_sf, aes(color = ASSET_High_text, alpha = 0.8))+ scale_color_viridis_d() + facet_grid(~ASSET_High_text)+
    theme_void() 
...
todo - drop na for ntee_text and only high asset
```{r}
multi_polygon_sf %>% filter(ASSET_High_text == "High Asset") %>% drop_na(NTEE_text)

asset_plot <- neighborhood_dem + 
    geom_sf(data = (multi_polygon_sf %>% filter(), aes(color = ASSET_High_text, alpha = 0.8))+ scale_color_viridis_d()+ facet_grid(~NTEE_text )+
    theme_void() 
```
