...
```{r}
df_simplified <- read_rds(here::here("data", "processed", "active_orgs.rds")) # org and shape/location info
```
...
It is not possible to distinguish a true zero from a zero due to not meeting the threshold of $50,000 for gross receipts and just not submitting. See [this guide](https://github.com/jhudsl/Baltimore_nonprofits_assets/blob/8094191b88bcef6503c132c7cc73408c12ddc916/Guide_to_Using_NCCS_Data_202.pdf), page 5 in the "minimum filing threshold" section.

It therefore makes sense to remove zero values and to report this caveat that the data is incomplete because many nonprofits that had gross receipts less than 50,000 are not included.
...
Adding to this, NA values might be considered ors with gross receipts less than 50000, as organizations are not required to report an amount if they have less than 50000.
...
NA and zero values may mean the nonprofit did not need to submit to the IRS.
...
```{r}
 df_simplified <- df_simplified %>%  
  mutate(ASSET_High = replace_na(ASSET_High, FALSE)) #NA values will be coded as not high asset for this analysis  (assuming that missing values are not high, because they would likely have a gross receipt of greater than 50000 and would be required to apply (although not everyone will still apply who should))
```
...
Now we will convert these to text as another variable and also create a log version of the asset amount to compress the scale, as plots and analysis may be easier to interpret if there are many values that are high or low.
...
Turns out this doesn't really work well, as we don't know if the values of 0  are real or for orgs that had a gross receipt of below 50,000 and didn't have to report total assets. So we will stick with our removal.
...
Look at log asset data for each NTEE type- **remember the caveat that there are many organizations that are not included because of NA or zero value F990_TOTAL_ASSETS_RECENT**. However, still we can see that some categories show a trend towards lower amount of assets even with this limited data.
...
```{r}
#recall that we can't differentiate not submitting as a zero value vs zero assets (true zero assets is also unlikely, solikely means they didn't submit)
#starting with a more basic model and checking assumptions
summary(glm(data = df_simplified_no_zero, F990_TOTAL_ASSETS_RECENT ~Percent_AA)) 
glm(data = df_simplified_no_zero, F990_TOTAL_ASSETS_RECENT ~Percent_AA) %>% plot(which = 1:3)
glm(data = df_simplified_no_zero, F990_TOTAL_ASSETS_RECENT ~Percent_AA + Population) %>% plot(which = 1:3) # plots indicate that there are outliers

# now try with log version of asset amount
hist(df_simplified_no_zero$ASSET_AMT_log)
summary(glm(data = df_simplified_no_zero, ASSET_AMT_log~Percent_AA)) 
glm(data = df_simplified_no_zero, ASSET_AMT_log ~ Percent_AA) %>% plot(which = 1:3) # looks much better

# now with population
glm(data = df_simplified_no_zero, ASSET_AMT_log ~ Percent_AA + Population) %>% plot(which = 1:3) # looks similar
...
```{r}
#nonparametric test - because the residuals looked skewed in the above qqplots
cor.test(df_simplified_no_zero$ASSET_AMT_log, df_simplified_no_zero$Percent_AA, method = "spearman", exact = FALSE)

sorted_data <- arrange(df_simplified_no_zero, desc(F990_TOTAL_ASSETS_RECENT))
glimpse(sorted_data)

## seem to have many outliers for asset amount

potential_outliers <- boxplot(df_simplified_no_zero$F990_TOTAL_ASSETS_RECENT, plot=FALSE)$out
df_simplified_no_zero_no_outliers <- filter(df_simplified_no_zero, !F990_TOTAL_ASSETS_RECENT %in% potential_outliers)

plot(df_simplified_no_zero_no_outliers$ASSET_AMT_log, df_simplified_no_zero_no_outliers$Percent_AA)

ggplot(df_simplified_no_zero_no_outliers, aes(Percent_AA, ASSET_AMT_log)) +
  geom_point(alpha = 0.2) +
  geom_smooth(method = "loess") # without outliers

ggplot(df_simplified_no_zero, aes(Percent_AA, ASSET_AMT_log)) +
  geom_point(alpha = 0.2) +
  geom_smooth(method = "loess") # with outliers

# notice a similar pattern with and without outliers
```


```{r}
# Check different models for scaling percent_AA, including or not including population 


lm1 <- lm(ASSET_AMT_log~ Percent_AA, data = df_simplified_no_zero)
lm2 <- lm(ASSET_AMT_log ~ poly(Percent_AA, 2), data = df_simplified_no_zero)

anova(lm1, lm2)
AIC(lm1)
AIC(lm2) # slightly lower and therefore possibly better model
summary(lm1)$adj.r.squared
summary(lm2)$adj.r.squared # slightly higher so possibly better model


# now log scale of Percent_AA vs poly_AA

lm1 <- lm(log(F990_TOTAL_ASSETS_RECENT) ~ log(Percent_AA +1), data = df_simplified_no_zero)
lm2 <- lm(log(F990_TOTAL_ASSETS_RECENT) ~ poly(Percent_AA, 2), data = df_simplified_no_zero)
anova(lm1, lm2) # significant, poly scaling of Percent_AA seems to be better than log
AIC(lm1)
AIC(lm2) # lower and therefore suggests this model is better

summary(lm1)$adj.r.squared
summary(lm2)$adj.r.squared # higher so possibly better model

#adding population seems to weaken the model 
lm1 <- lm(log(F990_TOTAL_ASSETS_RECENT) ~ poly(Percent_AA, 2), data = df_simplified_no_zero)
lm2 <- lm(log(F990_TOTAL_ASSETS_RECENT) ~ poly(Percent_AA, 2) + log(Population), data = df_simplified_no_zero)
anova(lm1, lm2) # not significant - adding population does not appear to be better
AIC(lm1)
AIC(lm2) # slightly higher, does not seem to be a better model
summary(lm1)$adj.r.squared
summary(lm2)$adj.r.squared # this is slightly higher than above

ggplot(df_simplified_no_zero, aes(poly(Percent_AA,2), log(F990_TOTAL_ASSETS_RECENT))) +
  geom_point(alpha = 0.2) +
...
Now we will compare with and without outliers and with and without zeros to see if our results are similar to before. 
```{r}


# trying again without possible outliers

lm2 <- lm(log(F990_TOTAL_ASSETS_RECENT) ~ poly(Percent_AA, 2), data = df_simplified_no_zero_no_outliers)
summary(lm2)
ggplot(df_simplified_no_zero_no_outliers, aes(poly(Percent_AA,2), log(F990_TOTAL_ASSETS_RECENT))) +
  geom_point(alpha = 0.2) +
  geom_smooth(method = "loess")

# trying again with orgs with zero asset value

lm2 <- lm(log(F990_TOTAL_ASSETS_RECENT+1) ~ poly(Percent_AA, 2) , data = df_simplified)
summary(lm2) #need plus 1 for log of zero
ggplot(df_simplified, aes(poly(Percent_AA,2), log(F990_TOTAL_ASSETS_RECENT+1))) +
  geom_point(alpha = 0.2) +
  geom_smooth(method = "loess")

```
It seems that there is a trend towards an association of assets with neighborhood Black population with what looks like reduced scaled assets for orgs in neighborhoods with a higher Black population.
...
**this does NOT include all organizations**
...
```
**this includes all organizations including those with NA values**
...
contingency_table <- table( df_simplified$Majority_AA, df_simplified$ASSET_High_text)
print(contingency_table)

independence_results <- chisq.test(contingency_table)
...
plot2 <- df_simplified %>% 
    mutate(NTEE_text = as_factor(NTEE_text),
         NTEE_text = forcats::fct_relevel(NTEE_text, "International Affairs", "Environment/Animals", "Arts", "Religious", "Health","Education", "Societal Benefit", "Human Services", "Other", "Unclassified" )) %>%
...
High_counts <- df_simplified %>% 
    mutate(NTEE_text = as_factor(NTEE_text),
        NTEE_text = forcats::fct_relevel(NTEE_text, "International Affairs", "Environment/Animals", "Arts", "Religious", "Health","Education", "Societal Benefit", "Human Services", "Other", "Unclassified" )) %>%
  summarize(count = n(), .by = c(NTEE_text, ASSET_High_text)) %>% filter(ASSET_High_text == "High Asset")
...
Count_by_neighborhood_type <- df_simplified %>% 
    mutate(NTEE_text = as_factor(NTEE_text),
        NTEE_text = forcats::fct_na_value_to_level(NTEE_text, level = "Unclassified"),   
        NTEE_text = forcats::fct_relevel(NTEE_text, "International Affairs", "Environment/Animals", "Arts", "Religious", "Health","Education", "Societal Benefit", "Human Services", "Unclassified")) %>%
  mutate(ASSET_CD_High = case_when(BMF_ASSET_CODE >= 5 ~ "High",
                                   BMF_ASSET_CODE < 5 ~ "Low")) %>%
  summarize(count = n(), .by = c(NTEE_text, Majority_AA)) %>% arrange(NTEE_text)

Count_by_neighborhood_type
```
...
neighborhood_AAperc%>% pull(Percent_AA) %>% hist(main = "Distribution of Black population percentages across Baltimore neighborhoods")

# get the neighborhood values after removing rows for nonprofits with NA or zero assets
neighborhood_AAperc_nozero %>% pull(Percent_AA) %>% hist(main = "Distribution of Black population percentages across Baltimore neighborhoods\n (removed neighborhoods with only zero or NA assets)")

```

## What % of people living in Baltimore neighborhoods live in majority black neighborhoods


```{r}
#start with the raw neighborhood statistical area data and use it to get population estimates 
neighborhoods <- read_csv(here::here("data", "Neighborhood_Statistical_Area_(NSA)_Boundaries.csv"))

Black_majority_neighborhoods <-neighborhoods%>% mutate(Percent_AA = 
           round(Blk_AfAm/Population*100, digits = 2)) %>%filter(Percent_AA>=50) %>% pull(Name)

length(Black_majority_neighborhoods) # number of black majority neighborhoods

Not_Black_majority_neighborhoods <-neighborhoods%>% mutate(Percent_AA = 
           round(Blk_AfAm/Population*100, digits = 2)) %>% filter(Percent_AA<50) %>% pull(Name)

length(Not_Black_majority_neighborhoods)

pop_in_black_neighborhoods <-filter(neighborhoods, Name %in% Black_majority_neighborhoods) %>% pull(Population) %>% sum()
# estimate of population living in Black majority neighborhoods
pop_in_black_neighborhoods

pop_not_in_black_neighborhoods <- filter(neighborhoods, !(Name %in% Black_majority_neighborhoods)) %>% pull(Population) %>% sum(na.rm = TRUE)

#estimate of population living in not Black majority neighborhoods
pop_not_in_black_neighborhoods

pop_in_black_neighborhoods/(pop_in_black_neighborhoods+pop_not_in_black_neighborhoods)*100

total_pop_est <- pop_not_in_black_neighborhoods+pop_not_in_black_neighborhoods
