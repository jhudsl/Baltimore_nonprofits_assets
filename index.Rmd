...
```{r}
 df_simplified<-df_simplified %>%
  ...
  mutate(ASSET_AMT_log = log(F990_TOTAL_ASSETS_RECENT))
```
...
st_join(Total_NTEE, High_counts) %>%
...
st_join(Total_NTEE, Count_by_neighborhood_type) %>%
   mutate("Percentage_of_each_code" = round(count/total_count *100, digits = 2)) %>%
  arrange(NTEE_text.x) %>%
  pivot_wider(names_from = Majority_AA, values_from = count:Percentage_of_each_code) %>% 
  select(NTEE_text.x, total_count, count_Yes, Percentage_of_each_code_Yes,  count_No, Percentage_of_each_code_No) %>% 
  
  
pivot_longer(-NTEE_text.x) %>%
